...
func (d *Daemon) StartAPI(ctx context.Context, bootstrap bool, initConfig map[string]string, joinAddresses ...string) error {
	...
	err = clients.Query(d.shutdownCtx, true, func(ctx context.Context, c types.Client) error {
		...
		err = d.sendUpgradeNotification(ctx, c.URL(), c.HTTP())
		if err != nil {
			return err
		}
		...
		if len(joinAddresses) > 0 {
			...
			if remote == nil {
				return fmt.Errorf("No remote found at address %q run the post-remove hook", c.URL().Host)
			}
...
func (d *Daemon) sendUpgradeNotification(ctx context.Context, path *url.URL, c *http.Client) error {
