...
type SchemaUpdate struct {
	updates map[updateType][]schema.Update // Ordered series of internal updates making up the schema
...
func (s *SchemaUpdate) Version() (internalVersion int, externalVersion int) {
	return len(s.updates[updateInternal]), len(s.updates[updateExternal])
}
...
func (s *SchemaUpdate) Ensure(db *sql.DB) (int, error) {
	...
	err := query.Transaction(context.TODO(), db, func(ctx context.Context, tx *sql.Tx) error {
		...
		if exists {
			...
			versions, err = query.SelectIntegers(ctx, tx, "SELECT MAX(version) FROM schemas WHERE type = 0 UNION ALL SELECT MAX(version) FROM schemas WHERE type = 1")
			if err != nil {
				return err
			}
		}
