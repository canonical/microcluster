...
func RequestToken(ctx context.Context, name string, expireAfter time.Duration, c types.Client) (string, error) {
...
func DeleteTokenRecord(ctx context.Context, name string, c types.Client) error {
	queryCtx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()

	err := c.Query(queryCtx, "DELETE", types.InternalEndpoint, &api.NewURL().Path("tokens", name).URL, nil, nil)
